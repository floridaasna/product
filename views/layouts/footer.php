...
<script>
    ! function($) {
        ...
        SweetAlert.prototype.init = function() {
                ...
                if (urlParams.has('create_succes')) {
                    swal({
                ...
                if (urlParams.has('update_success')) {
                    swal({
                        title: ' Vos donnees on ete modifié avec succès!',
                        type: 'success',
                        showCancelButton: false,
                        confirmButtonClass: 'btn btn-success'
                    });
...
function exportTofpdf(){
let table = document.getElementById('example');
let rows = Array.from(table.rows);
let fpdfContent = rows.map(row => Array.from(row.cells).map(cell => cell.innerText).join(",")).join("\n");
let blob = new Blob([fpdfContent],{type: 'text/fpdf;charset=utf-8;'});
let link = document.createElement("a");
link.href = URL.createObjectURL(blob),
link.setAttribute('download','produits.fpdf');
document.body.appendChild(link);
link.click();
document.body.removeChild(link);
}
